    ...
    class patch_Atlas : Atlas {
        ...
        public new List<MTexture> GetAtlasSubtextures(string key) {
            ...
            if (result == null || result.Count == 0) {
                Logger.Warn("Atlas", $"Requested atlas subtextures but none were found: {key}");
        ...
        public new MTexture GetAtlasSubtexturesAt(string key, int index) {
            if (orderedTexturesCache.TryGetValue(key, out List<MTexture> list)) {
                if (index < 0 || index >= list.Count) {
                    Logger.Warn("Atlas", $"Requested atlas subtexture that falls out of range: {key} {index}");
                    return GetFallback();
                }
                return list[index];
            }
            ...
            if (result == null) {
                ...
                if (fallback != null) {
                    ...
                    Logger.Warn("Atlas", $"Requested atlas subtexture that does not exist: {key} {index}");
                    return fallback;
                }
            }
            return result;
        ...
        public new MTexture this[string id] {
            ...
            get {
                if (!textures.TryGetValue(id, out MTexture result)) {
                    Logger.Warn("Atlas", $"Requested texture that does not exist: {id}");
                    return GetFallback();
                }
                return result;
            }
